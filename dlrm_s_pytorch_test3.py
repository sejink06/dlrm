...
class DLRM_Net(nn.Module):
    ...
    def create_emb_sj(self, m, ln):        
        with open(args.load_embedding, 'rb') as f:
            ...
            num_emb_tbl = f.read(8)
            num_emb_tbl = int(struct.unpack('q', num_emb_tbl)[0])
            # Read the indices to the embedding tables in bytes
            emb_idx = f.read(8 * num_emb_tbl)
            emb_idx = struct.unpack('%sq' % num_emb_tbl, emb_idx)
    ...
    def __init__(
        ...
        if (
            ...
            print("ln_bot:", ln_bot, flush = True)
    ...
    def apply_emb(self, lS_o, lS_i, emb_l):
        ...
        for k in range(len(lS_i)):
            ...
            E = emb_l[k]
            #print("thread count before E:", threading.active_count(), flush = True)
            V = E(sparse_index_group_batch, sparse_offset_group_batch)        
            #print("thread count after E:", threading.active_count(), flush = True)
            ly.append(V)
    ...
    def emb_lookup_cpp(self, table_num, vector_num):
        ...
        t3 = time.time()
        #emb_vec = struct.unpack('%sf' % args.arch_sparse_feature_size, emb_vec[:4 * args.arch_sparse_feature_size])  
        #t4 = time.time()
        emb_vec = torch.Tensor(emb_vec)
        t5 = time.time()
        offset_[0] += (t2 - t1)
        ...
        for k in range(len(lS_i)):
            ...
            for i in range(len(sparse_offset_group_batch)):
                ...
                if i == len(sparse_offset_group_batch) - 1:
                    if "disk_direct" in args.mem_size or "ramdisk_cpp" in args.mem_size:
                            emb = self.emb_lookup_cpp(k, int(sparse_index_group_batch[i]))
                    else:
                        emb = self.emb_lookup(k, int(sparse_index_group_batch[i]))
                    t5 = time.time()
                    #embeddings[str(sparse_index_group_batch[j])] = emb
                    t6 = time.time()
                    summed += emb
                    t7 = time.time()
                    dict_save[0] += (t6 - t5)
                    ...
                    for j in range(sparse_offset_group_batch[i], sparse_offset_group_batch[i + 1]):
                        """
                        if str(sparse_index_group_batch[j]) in embeddings:
                            t3 = time.time()
                            summed += embeddings[str(sparse_index_group_batch[j])]
                            t4 = time.time()
                            dict_sum[0] += (t4 - t3)
                            dict_sum_count += 1
                        else:
                        ...
                        dict_save[0] += (t6 - t5)
                        sum_[0] += (t7 - t6)
                        #print(i, summed.size(), flush = True)
                t8 = time.time()
                V = torch.cat((V, summed))
                t9 = time.time()
                cat[0] += (t9 - t8)
            #print("tensor shape:", V.size(), flush = True)
            t10 = time.time()
    ...
    def interact_features(self, x, ly):
        if self.arch_interaction_op == "dot":
            ...
            (batch_size, d) = x.shape
            T = torch.cat([x] + ly, dim=1).view((batch_size, -1, d))
            ly = torch.cat(ly, dim = 1).view((batch_size, -1, d))   
            total_sum_count[0] += len(ly)
            t1 = time.time()
            Ly = torch.sum(ly, dim = 1)
            t2 = time.time()
            #print("ly.shape:", ly.shape, flush = True)
            feat_interact[0] += t2 - t1
            #print("ly length:", len(ly), flush = True)
            R = torch.sum(T, dim=1)
        else:
            sys.exit(
...
if __name__ == "__main__":
    ...
    # added by sejin
    parser.add_argument("--mem-size", type=str, default='full')
    ...
    if "disk_direct" in args.mem_size:
        #fd = os.open(args.load_embedding, os.O_RDONLY | os.O_DIRECT)
        embedding.set_direct_io(args.load_embedding, True)
    elif "ramdisk_cpp" in args.mem_size:
        #fd = os.open(args.load_embedding, os.O_RDONLY)
        embedding.set_direct_io(args.load_embedding, False)
    else:
        f = open(args.load_embedding, 'rb')
     
    with torch.autograd.profiler.profile(args.enable_profiling, use_gpu) as prof:
        print("thread count at a:", threading.active_count(), flush = True)
        while k < args.nepochs:
            ...
            train_ld_len = torch.load('saved_sum/train_ld_len.pt')
            
            #쿵교수님 실험 끝나고 복구시키기
            
            for j in range(train_ld_len):
                X = torch.load('saved_sum/X' + str(j) + '.pt')
                lS_o = torch.load('saved_sum/lS_o' + str(j) + '.pt')
                lS_i = torch.load('saved_sum/lS_i' + str(j) + '.pt')
                T = torch.load('saved_sum/T' + str(j) + '.pt')      
            ...
            #for j, (X, lS_o, lS_i, T) in enumerate(train_ld):
                ...
                if should_print or should_test:
                    ...
                    if args.save_time:
                        t1s_ = np.array(t1s) * 1000
                        t2s_ = np.array(t2s) * 1000
                        t3s_ = np.array(t3s) * 1000
                        t4s_ = np.array(t4s) * 1000
                        seek = np.array(seek) * 1000
                        read = np.array(read) * 1000
                        unpack = np.array(unpack) *1000
                        to_tensor = np.array(to_tensor) * 1000
                        zeros = np.array(zeros) * 1000
                        dict_sum = np.array(dict_sum) * 1000
                        sum_ = np.array(sum_) * 1000
                        dict_save = np.array(dict_save) * 1000
                        cat = np.array(cat) * 1000
                        offset_ = np.array(offset_) * 1000
                        pread = np.array(pread) * 1000
                        view = np.array(view) * 1000
                        append_ = np.array(append_) * 1000
                        feat_interact = np.array(feat_interact) * 1000
                        total_sum_count = np.array(total_sum_count) *1000                                       
                        
                        with open('pickles/t1s' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(t1s_, f, pickle.HIGHEST_PROTOCOL)
                            
                        with open('pickles/t2s' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(t2s_, f, pickle.HIGHEST_PROTOCOL)
                            
                        with open('pickles/t3s' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(t3s_, f, pickle.HIGHEST_PROTOCOL)
                            
                        with open('pickles/t4s' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(t4s_, f, pickle.HIGHEST_PROTOCOL)  
                        with open('pickles/seek' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(seek, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/read' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(read, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/unpack' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(unpack, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/to_tensor' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(to_tensor, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/zeros' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(zeros, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/dict_sum' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(dict_sum, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/sum_' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(sum_, f, pickle.HIGHEST_PROTOCOL) 
                        with open('pickles/dict_save' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(dict_save, f, pickle.HIGHEST_PROTOCOL)
                        with open('pickles/cat' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(cat, f, pickle.HIGHEST_PROTOCOL) 
                            
                        with open('pickles/offset_' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(offset_, f, pickle.HIGHEST_PROTOCOL) 
                            
                        with open('pickles/pread' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(pread, f, pickle.HIGHEST_PROTOCOL) 
                            
                        with open('pickles/view' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(view, f, pickle.HIGHEST_PROTOCOL) 
                        
                        with open('pickles/append_' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(append_, f, pickle.HIGHEST_PROTOCOL) 
                        
                        with open('pickles/feat_interact' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(feat_interact, f, pickle.HIGHEST_PROTOCOL) 
                            
                        with open('pickles/total_sum_count' + args.mem_size + '.pickle', 'wb') as f:
                            # Pickle the 'data' dictionary using the highest protocol available.
                            pickle.dump(total_sum_count, f, pickle.HIGHEST_PROTOCOL) 
                            
                        print('average:', np.average(t1s_), np.average(t2s_), np.average(t3s_), np.average(t4s_), sep = '    ')
                        print('max:', np.max(t1s_), np.max(t2s_), np.max(t3s_), np.max(t4s_), sep = '    ')
                        print("dict_sum_count:", dict_sum_count)
                        print("dict_save_count:", dict_save_count)
                ...
                if should_test and not args.inference_only:
                    ...
                    if is_best:
                        ...
                        if not (args.save_model == ""):
                            ...
                            header = [ln_emb.size, (ln_emb.size + 1) * 8]
                            emb_bytes = bytes()
                            """
                            ...
                            emb_file = struct.pack('%sq' % len(header), *header)
                            emb_file += emb_bytes
